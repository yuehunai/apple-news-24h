...
4. For Chinese daily digests such as morning briefings, extract each distinct Apple-specific subitem as its own candidate. Keep non-Apple subitems out even when the roundup headline, metadata, or nearby paragraph mentions Apple as unrelated background.

When a source feed is complete but still misses individual current-window posts from a high-volume source, use a structured first-party endpoint such as a WordPress posts API before broad web search. Treat API/listing timestamps as discovery hints only; the detail page's precise published time remains authoritative. Current OS feature guide pages may be candidates when the detail page timestamp is inside the window and the page contains concrete new beta, OS, built-in app, or platform feature changes; evergreen guide pages without a current timestamp or material update remain excluded.
...
- Apple hardware launch, roadmap, or rumor stories remain hardware/product news when the subject is an Apple device and the material details are design, display, camera, chip, modem, sensor, battery, Touch ID/Face ID, storage, or release timing, even if the article also discusses health, Apple Intelligence, Siri, OS support, or other software-adjacent capabilities. Apple product price increases, component-cost pressure, memory/storage shortages, supply-cost pass-through stories, Apple explanations or responses to those price changes, and Apple product/supplier data-leak stories involving schematics, data sheets, source files, modem files, board designs, factory breaches, or dark-web file leaks are also hardware/product news, not software/system news.
...
- Use each event's `must_include_facts` and `key_facts` before writing the brief; `must_include_facts` must appear in that event's final bullet, and `key_facts` are mandatory source material extracted generically from numeric paragraphs, list items, tables, feature lists, official explanations, and official responses, not optional background. Preserve material enumerations from official sources, including fraud/safety report figures, App Store review counts, eligible countries or regions, device model lists, feature names, terms, dates, rollout limits, localized features, input methods, regional rollouts, compatibility lists, feature lists, stated reasons, acknowledgements, commitments, follow-up positions, and other implementation details surfaced in `title`, `summary`, `must_include_facts`, or `key_facts`; compress phrasing when needed, but do not collapse the list, official rationale, or response substance into a vague summary.
- Coverage comes before brevity: include every eligible event, not just a hand-picked top set. Treat the current JSON `events` as an eligible-event queue, `required_final_brief_titles` and `final_brief_queue` as the coverage checklist, and `brief_output` / `final_brief_markdown` as compact current-run scaffolds, not as loose source material for freeform recombination; preserve scaffold bullet boundaries by default. A smaller event that has a concrete Apple product, system, service, local-language, regional-availability, developer-platform, privacy/security, health/accessibility, or built-in-app change must still appear in the final brief when it passed inclusion. Before finalizing, perform a current-JSON coverage pass: every bullet must map to one or more event ids from the just-written JSON, no bullet may come from older output or memory, and every `required: true` queue item must be accounted for unless it was explicitly excluded or merged as duplicate coverage after source review. Do not drop eligible granular events just because a same-day major OS, WWDC, service, or platform story is more prominent or speculative; when volume is high, shorten wording within each event's bullet instead of combining independent events into broad theme paragraphs. Keep independent Apple Wallet/Digital ID identity verification, Apple Cash split-payment, App Store policy/commission, Apple Pay/Card promotion, and Apple services offer items separate unless source review proves they are duplicate coverage of the same action.
- Speculative wording such as `may`, `could`, `reportedly`, `消息称`, or `或将` is not an omission reason when the crawler kept the event in `events`; write it with proper attribution instead of silently dropping it.

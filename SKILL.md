...
Current JSON output may also include `required_final_brief_titles`, `final_brief_queue`, `brief_output`, and `final_brief_markdown`; JSON runs may write an adjacent `*.brief.md` coverage scaffold. Use these fields only as coverage and bullet-boundary checklists. Do not copy scaffold prose verbatim, do not replace `events[].summary` and `events[].key_facts` with the scaffold, and do not add a queue item when source review proves it is not a direct Apple software, hardware, service, platform, policy, retail, legal, company, or ecosystem-impact event.
...
6. Use JSON `events` as the final-brief queue. When present, compare `required_final_brief_titles`, `final_brief_queue`, `brief_output`, `final_brief_markdown`, or the adjacent `*.brief.md` scaffold against the final answer to prevent omissions, while still drafting from full event summaries, key facts, must-include facts, and source pages. Review `deferred_events` only for clearly misclassified direct Apple actions; otherwise leave weak Apple-adjacent stories out of the brief.
...
- Before finalizing, compare the event count in JSON against the number of bullets you wrote. If you skipped an `events` item, it must be because of a clear exclusion rule, not because it was single-source, speculative, rumor-framed, niche, less important, competitor-adjacent, third-party-adjacent, or less prominent than same-day major news. If the bullet count is lower than the number of eligible JSON `events`, the difference must come only from explicit duplicate-event merges after source review; do not create combined theme paragraphs that cover multiple independent JSON events.

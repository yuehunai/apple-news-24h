...
- Exclude tutorials, buying guides, routine deals, pure roundups without new information, podcasts without new reporting, simple reposts, affiliate roundups, and non-Apple product stories even when they mention Apple products. Do not let competitor, third-party, or partner background terms override a clear Apple-subject story: if the title or article lead is about an Apple platform, Apple OS, Apple service, Apple official accessory, Apple legal action, or Apple developer tool, classify by that Apple action.
- Do not treat official Apple Card, Apple Pay, App Store, Apple services, or Apple retail/customer acquisition promotions as routine deal posts when they reveal a new or materially changed Apple business offer; include enough terms, eligibility, value, timing, and uncertainty details. Treat Apple official accessory availability changes, including Apple Store removal, regional unavailability, discontinuation, or suspected discontinuation, as hardware/product news rather than routine retail or third-party market background.

### Event Grouping And Coverage

- Treat JSON `events` as the final-brief queue. Default to one eligible JSON event becoming one final bullet; do not treat separate events as loose facts for freeform recombination.
- Merge final bullets only after source review shows duplicate coverage of the same core subject and action: the same product, app, service, component, feature, policy, legal/company move, hardware item, region rollout, or official announcement. Shared Apple context, OS version, WWDC timing, product family, Apple Intelligence theme, source timing, or broad event kind is never enough.
...
- Preserve granular events during high-volume WWDC, OS-preview, and roadmap runs. Built-in apps, OS components, platform APIs, developer tools, service/content items, hardware rumors, regional rollouts, compatibility lists, and feature lists stay separate when their concrete action differs. For example, Mail search, Weather forecasts, Notes features, Shortcuts generation, Recovery mode, Apple Wallet, Xcode integration, Apple TV Remote, and App Store subscriptions are separate topics unless the sources explicitly report one combined change.

...
- 新增以标题和导语为优先依据的事件身份及具体法律案件主题，避免正文背景、演员名单、产品枚举和来源名称后缀把无关的 Apple 诉讼、服务、AI 或硬件报道连接到同一事件。
- 改进 OLED iPad mini 显示规格、AirPods 测试版固件、Apple Arcade 发布、欧盟电池豁免和 Apple 硬件产线变化等同动作报道的跨源归并，同时保持多产品面板路线图和不同 Apple TV 内容相互独立。
- 改进端侧 AI 技术评估、iPhone 运营商锁定政策、分析师评级与目标价调整、Siri AI 可用性、Apple 市场份额报告和区域平台监管等直接 Apple 动作的相关性判断。
- 收紧全弱相关聚合事件自行升级、推测性法律参与、第三方 AI 模型或应用发布、竞品对比、产品前瞻、教程、观点及无新增 Apple 动作的功能汇总过滤。
- 更新兜底规则，将回归测试从 528 项扩展至 555 项；联网基线对比保持来源和 URL 覆盖不缩窄、运行时间低于目标，并在纯净验证中得到 21 个事件且无聚类警告。

...
- 新增按设施类型、地区及具体启用或扩展动作建立的跨语言第一方设施身份，使教育中心与制造背景、捐款、人员配置及无关社区项目保持分离。
- 改进官方活动排期与邀请函图案解读、Mac mini 路线图与待交付订单升级及技术披露之间的事件边界，并避免把事实范围并非专属于 `Ultra`、Pro、Air 等子型号的多产品报道错误投影为特定型号事件。
- 改进标题和导语的动作裁决，使明确报道当前 Apple 产品发布或调价的上手体验稿可以并入直接第一方事件，同时让第三方二手价格研究、折抵估值、购物替代方案和命令式购买建议继续留在延后队列。
- 修复普通教育和培训文本被误读为 Apple Intelligence 证据的问题，并让明确的硬件细节披露保持在正确产品分类中而不重新打开宽泛旧聚类。
- 更新兜底规则并将回归测试由 1,247 项增加到 1,254 项；并发联网验证在 76.94/77.50 秒内保持发现、详情选择、文章数量及全部 69 次来源 URL 引用一致，纯净子代理验证产生零条合并警告，并在无需人工重组或从延后队列提升的情况下保留全部 47 次主事件来源引用。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260828_structured_boundaries.py` 的按日期命名协调测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、发布会活动、第一方设施、活动解读、内容形态边界和相关性分层提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

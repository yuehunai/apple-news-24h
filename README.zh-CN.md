...
- 改进桥接文章和多动作文章、具体 Mac 产品路线图、Apple 融资计划及限制机制、Apple TV 公告，以及细粒度 iPhone 或系统变化的语义事件身份，避免相关背景再次压缩不同的具体动作。
- 改进第三方 App 更新、兼容性报道、竞品模仿、分析师前瞻、配件和比较型报道的标题与导语相关性判断，同时保留直接 Apple 关系和明确的生态互操作事件。
- 新增更严格的最终简报边界：除非标题和导语证明两个事件具有相同规范主体与动作，否则每个 `event.id` 必须保持独立；从 `deferred_events` 推翻既有弱相关原因时，也必须具备标题或导语级直接 Apple 动作证据。
- 更新 `SKILL.md` 和 `references/news_policy.md`，补充机械事件台账、来源链接、CarPlay、因果链、共享来源及产品路线图约束，提高自动化输出可靠性。
- 增加回归测试，将测试数量从 665 项扩展至 706 项；并行联网对比完整保留 81 个文章 URL 和各来源覆盖，未产生聚类警告，运行性能也未回退。

...
- 新增官方教育优惠、AppleCare+ 调价、运营商分期设备锁定、国行 Apple Intelligence 审批、地图广告政策、AI 芯片收购、区域平台质询和免税活动等直接事件身份及同动作跨源归并。
- 改进详情页标题与导语优先级，避免 Feed 摘要、相关文章、正文比较和非 Apple 发现上下文把无关的市场、法律、供应商、应用或分析师报道升级或合并。
- 改进直接 macOS 恶意软件、Apple Silicon 操作系统与驱动认证、Apple 单一产品预测、分析师观点和多厂商报告的相关性与事件边界，使弱相关或背景条目不再进入必收事件。
- 新增结构化数据序列提取，在 `must_include_facts` 中完整保留按型号或地区列出的价格、支持、资格和状态数据，同时避免将带冒号的叙述段落误判为强制数据行。
- 更新代理与兜底规则，将回归测试从 555 项扩展至 581 项；并行联网对比保持候选、文章和 URL 覆盖不缩窄、无聚类警告，运行时间维持在约 120 秒目标附近。

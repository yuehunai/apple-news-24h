...
- 改为由结构化协调器对旧召回种子执行最终裁决，通过标题与导语的动作归属及具体主体边界，在最终输出前拆开服务、路线图、管理层、系统发布和内容报道中的宽泛桥接。
- 新增版本化 OS 测试版与发布候选版、第一方购物助手和 Home Hub 功能、Apple TV 内容与体育动作、具名钱包交通卡上线、内部 AI 服务器、发布会日程及当前硬件发布窗口的结构化身份。
- 改进多产品报道投影，按句子限定主体、动作、日期和支持事实，在保留可独立报道的产品发布同时，拒绝把背景提及及缺少自身事件证据的指代式同期发布描述生成独立事件。
- 收紧第一人称产品前瞻、非 Apple 跑分或对比报道及第三方定制 Apple 设备的相关性判断，同时继续保留直接 Apple 平台发布、硬件路线图和来源完整的同动作报道。
- 更新兜底规则并将回归测试由 1,156 项增加到 1,190 项；并发联网验证在 69.8／73.2 秒内保持发现和详情选择数量完全一致及全部 96 个来源 URL，纯净子代理验证保留全部 31 个主事件、维持 18 个弱候选延后，且无需人工纠正事件边界。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260825_authoritative_structural_pipeline.py` 的按日期命名协调测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、种子证据、内容形态边界和相关性分层提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

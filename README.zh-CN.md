...
- 改进直接 Apple OS 组件、当前系统 App 指南、watchOS 内置 App 变化，以及 Apple 获准使用受监管芯片或数据中心设备等政府行动的标题与导语优先判定。
- 收紧不同 OS 平台内置 App、折叠 iPhone 电池容量与电池法规、产品机模、OpenAI 商业机密诉讼和 iPhone 产量预测的事件边界。
- 改进折叠 iPhone 电池与机模、Apple 市场份额研究、法律行动和产量指标的跨来源归并，同时保留不同产品动作和具体数字事实。
- 收紧个人功能体验、列表式传闻汇总，以及没有新增 Apple 动作的常规第三方桌面客户端更新过滤。
- 修复直接 Apple 市场报告和实体 Apple Store 迁址分类，避免比较背景或复数零售店写法导致误降级或错误归入软件板块。
- 更新兜底规则文档并将回归测试扩展至 477 项，同时保持联网发现范围不变，抓取耗时维持近期性能目标。

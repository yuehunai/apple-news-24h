...
- 改进 deferred 事件回拉规则，使 Apple/Epic 最高法院上诉、Siri AI / 欧盟 DMA 会谈等直接 Apple 法律与监管事件稳定进入主简报。
- 改进事件聚类，使 Tim Cook / 欧盟 Siri AI 报道可跨软件、政策和领导层角度归并，同时保持不同 App Store 法律事项相互独立。
- 改进产品评论和分析文弱过滤，使 MacBook Ultra 观点类文章不再进入必收简报，同时保留有来源支撑的硬件路线图传闻。
- 改进 Apple Pay 与 App Store 支付分类，避免法律文章中的 Apple Pay 字样、抽佣比例或 `interest` 等词把 App Store 诉讼误降级为第三方金融服务候选。
- 更新兜底策略说明，并增加监管会谈、法律上诉、产品评论排除、竞品显示面板背景文和聚类边界回归测试。

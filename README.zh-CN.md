...
- 新增 Apple 智能眼镜与当前有明确消息来源的 Apple Watch 路线图报道的标题、导语产品及动作边界，同时将独立 AirPods 趣味项目、通用兼容性清单和竞品主导的对比报道保持为弱相关。
- 新增复合 Apple TV 文章的逐标题投影，使分别命名的作品与生命周期动作保持为独立事件，同时保留共享的来源 URL 和兼容事实。
- 改进周年 iPhone 代际归一化和具体跨语言聚类，使同一产品的直接报道正确合并，并防止对比背景或相邻产品重新定义事件。
- 在 JSON 输出中新增来源级 `title`、`published_at` 和 `primary_fact` 证据，并从首要证据中剔除历史背景，帮助自动化 agent 保留有效来源链接和当前来源特有事实。
- 新增通用强事件升级的结构化标题与导语证据要求，避免仅在正文出现的同形词、历史产品示例和偶然 Apple 引用把无关文章错误升级。
- 改进 9to5Mac 文末推广清理，更新手动兜底策略，并将回归测试从 741 项增加到 753 项；并行联网验证保持发现范围不缩窄、无聚类警告，且相较同时间窗基线缩短了运行时间。

...
- 新增更严格的标题与导语事件身份，覆盖产品周年、投票、官方翻新商店变化、Apple 官方活动挑战、实质性 App Store 下架或恢复，以及包含多个产品的 Mac 路线图。
- 改进确定性事件重整，避免泛化的 Apple、产品、路线图、价格、促销和第三方背景桥接无关动作，同时让同一 Mac Pro 里程碑或翻新商店更新的多语言报道保留全部匹配来源。
- 新增复合路线图文章的产品级投射，让 MacBook Ultra、M6 MacBook Pro、AirPods Ultra 和折叠 iPhone 动作保持独立，不再折叠为宽泛硬件聚类。
- 改进主题范围内的摘要和关键事实抽取，使用标题加首个非重复导语确定主事件，支持无空格中文句子边界，并排除来自不兼容系统平台、版本、产品或背景事件的事实。
- 改进相关性处理，使直接 Apple Wallet 功能、App Store 执法结果、Apple Watch 官方挑战和实质性认证翻新商店上新保持强相关，同时将投票、例行优惠、雇主资产处置和无直接 Apple 动作的第三方更新留在延后队列或排除。
- 更新手动兜底规则，并将回归测试从 903 项增加到 928 项；并发联网验证保持候选、文章和 57 个 URL 覆盖完全一致、聚类警告为零，运行时间由 115.15 秒降至 110.59 秒。
...
- `tests/test_event_identity_architecture.py`、`tests/test_20260805_event_reconciliation.py` 和 `tests/test_20260806_relevance_boundaries.py` 为身份提取、匹配、重整和相关性边界提供聚焦的回归测试；现有爬虫测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

...
- 改进标题主动作归并，让完整的标题主体和谓词优先于正文背景生成的聚类键，在保留同动作来源覆盖的同时，避免重新混入无关事件。
- 修复 Apple 服务商业模式审查及具名人物出席或缺席发布会的跨语言归并，将拟议商业模式调整与已实施收费、功能更新分开，并将参与报道与日程安排、管理层任命分开。
- 改进操作系统兼容性身份解析，将项目名与发布动词分离，并绑定目标芯片代际和发布阶段；保持组件驱动进展独立，将具有具体 Apple 平台影响的系统适配归为软件生态新闻。
- 保持提及 Apple 产品或发布会的第三方应用玩法推荐处于延后队列，保留真正的第一方应用动作，并防止观点、播客体裁及无关导语细节错误提升或桥接新闻事件。
- 移除重复的兼容性投影逻辑并更新兜底规则说明，保持 skill 工作流程、CLI、来源发现、详情页覆盖和输出结构不变。
- 新增 16 项聚焦回归测试，将测试总数由 1,455 项增加到 1,471 项；并发联网验证保持各来源计数一致，保留全部 19 个来源 URL，耗时为 73.58/68.34 秒；纯净子代理验证输出六条与脚本事件一一对应的简报，完整保留 12 个有效来源链接，无需人工重新聚类或提升延后事件。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260907_action_grammar.py` 的按日期命名测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、发布会活动、第一方设施、活动解读、内容形态边界、相关性分层、产品生命周期、实际部署、量化公司报告、详情页准入和官方叙述事实提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

...
- 新增具体的 Apple 芯片动作边界，将 M6 产品线换代、M7 Ultra 内存容量、取消项目的研究成果转移及 M8 制程规划保持为独立事件，同时继续归并同一动作的跨源报道。
- 改进 Apple Pencil 路线图和 Apple Store 员工设备部署的跨源归并，避免法规或系统背景拆开同一硬件动作，或将其错误归入软件新闻。
- 收紧犯罪汇总、第二人称购买建议、夸张的多品牌激活量对比，以及主要以 Apple 为参照的竞品芯片或制程发布等弱相关内容过滤。
- 改进公司组织变动识别，避免通用的技术架构措辞覆盖直接的 Apple 芯片研究、容量或路线图报道。
- 更新兜底规则文档并将回归测试扩展至 490 项；联网基线对比保持候选、详情、文章及 URL 覆盖完全一致，同时缩短抓取耗时。

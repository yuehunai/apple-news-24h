...
- 新增正式版系统更新的平台级事件身份，使 iOS/iPadOS 配套报道可以合并，同时让 macOS、watchOS、tvOS、visionOS、跨平台安全公告和旧版 macOS 更新保持为独立事件。
- 收紧安全事件匹配，要求共享具体版本、公告、漏洞、组件或攻击路径，避免泛化的安全词把 Apple 正式版更新、旧版补丁和第三方应用漏洞错误连接。
- 改进 App Store 假钱包诉讼、Apple TV 制作进展、Apple 市值排名、visionOS 发布及旧版 macOS 安全更新的跨来源和跨语言聚类，并保留所有有效来源。
- 改进标题与导语的动作归属判断，使观点文章、个人案例、推广配件、运营商故障、治安新闻和第三方 Wallet 服务在没有 Apple 新动作时保持弱相关，同时保留第一方 AppleCare 变化。
- 更新手动兜底策略，并将回归测试从 753 项增加到 763 项；并行联网验证保留全部 80 个已发现来源 URL、没有聚类警告，且运行性能没有回退。

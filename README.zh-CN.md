...
- 新增结构化设置建议和单用户维修／变通方案内容形态，使从用户症状出发的教程与个案修复保持延后，除非标题和导语同时报告当前 Apple 发布、支持或维修计划动作。
- 改进中英文不同标题语序下的竞品发布动作主体识别，使 Apple 产品仅作为比较背景，同时保留直接 Apple 硬件、定价、维修和量化市场动作。
- 修复 Apple 产品名与中文字符紧邻时的第一方标题前缀解析，并阻止标题分隔符后的竞品比较从句覆盖标题主导的 Apple 主体。
- 更新设置建议、用户维修轶事及比较措辞位于 Apple 产品名前后时的竞品定位兜底规则。
- 将回归测试由 1,152 项增加到 1,156 项；并发联网验证在约 50 秒内保持逐来源发现、详情选择、文章和全部 14 个来源 URL 覆盖完全一致，同时把 3 个误升主事件移入 `deferred_events`，纯净子代理验证完成全部 13 个事件 ID 落账且聚类警告为 0。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260823_editorial_action_owner_boundaries.py` 的按日期命名协调测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、种子证据、内容形态边界和相关性分层提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

...
- 改进 Apple 产品涨价事件处理，使用动态 `key_facts` 上限、结构化价格变动分桶、市场和分析师反应、后续范围事实，以及压缩后的官方回应，使大型涨价聚类保留关键数字细节。
- 改进结构化列表抽取，使中英文来源的长产品表不会截断后半段，例如 Vision Pro、HomePod、Apple TV、区域价格变化和相关产品范围细节。
- 改进 Apple 在线商店状态、官方翻新产品上架、直接改善 Apple 设备互通的第三方平台更新，以及 Apple 硬件涨价或促销跟进的相关性和聚类，同时保持常规零售优惠为弱相关。
- 收紧价格事件事实过滤，避免 Siri/prompt、播客、导购或优惠文本等包含大量数字的无关段落进入涨价摘要。
- 更新高流量涨价、官方商店或翻新、第三方平台互通、结构化事实保留，以及直接 Apple 事件人工拉回的兜底规则与 skill 指引。
- 增加涨价事件 `key_facts`、结构化价格表、商店和翻新处理、互通更新、涨价/促销拆分、弱促销过滤和大型聚类事实保留的回归测试。

...
- 新增更具体的硬件路线图 facet 和保守混合主题拆分，使 iPhone Air 后续机型、折叠 iPhone 渲染图爆料以及相关 tag/list 页面不再被压成同一事件。
- 改进 iOS 性能优化覆盖，可将 9to5Mac、IT之家、快科技关于 40 多项优化的数据型报道合并，同时保持无关未来硬件测试独立。
- 改进 Apple 产品涨价事件聚类，增加跨语言价格、成本、内存、存储和 Tim Cook 锚点，并避免同一个全球涨价事件产生误导性的多地区警告。
- 聚焦 IT之家等早报/汇总文章中的 Apple 相关条目，避免 DeepSeek、微信支付等无关日报主题占据标题、摘要或 `key_facts`。
- 增加 term、topic facet 和研究候选判断缓存，使高流量抓取中的事件归并保持响应速度。
- 增加回归测试，覆盖早报聚焦、摘要级性能优化合并、产品涨价聚类、硬件路线图拆分和地区警告处理。

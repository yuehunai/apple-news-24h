...
- 新增受保护的第三方 Apple 平台可用性判断，让常规 App 或服务上架保留在 `deferred_events`，同时保留 Apple 平台批准和生态互通类事件进入简报的能力。
- 改进高新闻量 WWDC 和 OS 预览聚类，为 App Store 政策、订阅组合、发现功能、Apple 服务、AirPods 固件和设置、Sidecar、iPhone 镜像、壁纸、macOS 性能反馈、MacBook 内存或本地 AI 报告增加主题锚点。
- 改进最终简报指引，长简报压缩时仍将硬件路线图线索、硬件规格传闻、OS 功能变化、服务政策变化和 App / 服务上线作为独立事件处理。
- 改进来源清理，在摘要和 `key_facts` 抽取前移除文章尾部的联盟推广推荐区块。
- 修复 macOS 性能反馈分类，让性能和 bug 相关表述保留在软件与系统板块，而不是误入安全或硬件分类。
- 增加第三方弱相关延后、推广区块清理、微事件聚类、macOS 性能分类、MacBook 内存 / 本地 AI 拆分和最终来源链接行为的回归测试。

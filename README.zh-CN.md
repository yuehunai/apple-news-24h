...
这是本项目第一个正式大版本。skill 的迭代过程正好遇上 WWDC 新闻高峰，相当于一次真实压力测试：高密度 Apple 报道更快暴露了来源发现、事件聚类和最终简报压缩中的问题，也推动了本版本的集中修复和加固。

- 改进 OS 和 WWDC 微事件聚类，为通信框架、FaceTime 摄像头功能、Time Machine / AFP 变化、启动卷检测、Rosetta 支持和 Mac 硬件可靠性报告增加组件级锚点。
- 收紧最终简报指引，除非人工核验确认是同一核心事件的重复报道，否则独立 JSON `events` 应保持为独立要点。
- 改进直接 Apple 主体判断，避免竞品、第三方或合作方背景词压制 Apple OS、开发者工具、法律、服务、官方配件和生态互通类新闻。
- 将来源清理细节保留在兜底策略中，同时精简 `SKILL.md`，让其更聚焦自动化流程和最终输出约束。
- 增加开发者工具相关性、官方配件可用性、窗口内详情优先级、第三方弱相关延后和 OS 组件拆分的回归测试。

...
- 新增基于标题与导语的事件身份，覆盖多产品目录撤下与机模披露、Apple 设备受监管的实际部署、Apple 公司量化业绩、官方产品生命周期变化及第一方命名对象变更。
- 改进 CEO 任命、薪酬、员工备忘录、社交账号动作、职责退出、Apple 地图更名、法律文件阶段、Vision 团队调整、折叠设备 MagSafe 支持及 Mac App Store Intel 支持政策的跨语言归并。
- 收紧结构化边界，避免产品生命周期报道桥接到显示功能新闻，使不同高管与法律动作保持独立，并让当前系统功能在存在硬件背景时仍归入软件分类。
- 改进第三方 Apple 平台应用、领导力评分、历史采访、非 Apple 对比、个人上手文章及多品牌价格报道的相关性判断，同时保留直接 Apple 动作、量化公司报告和实际运营部署。
- 更新 skill 与兜底策略说明，并新增 51 项聚焦回归测试，将测试总数由 1,279 项增加到 1,330 项；纯净子代理验证生成 30 个主事件和 23 个延后事件，完整保留 75 个主事件来源 URL，且无需人工重新聚类或从延后队列提升事件。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260902_current_report_and_deployment_boundaries.py` 的按日期命名协调测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、发布会活动、第一方设施、活动解读、内容形态边界、相关性分层、产品生命周期、实际部署和量化公司报告提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

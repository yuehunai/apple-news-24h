...
- 新增规模化机构采用 Apple 硬件、Apple 产品供应约束、iPhone 组件成本预测和 Apple 智能眼镜健康平台的标题/导语事件身份，使直接产品动作不会被竞品与行业背景压低。
- 改进跨语言 MacBook 缺货、MacBook 过热故障和智能眼镜健康报道的同动作归并，同时保持终端售价预测、物料成本分析、产品故障和零售促销相互独立。
- 收紧竞品跟进、宽泛 SoC 或组件市场报告、静态 Apple 对比指标及带零售价促销文章的弱相关判断，即使标题没有显式出现“优惠”或“促销”字样也能正确降级。
- 改进标题主导的硬件边界，使当前 Apple 供应紧张和量化成本报告保持强相关，同时避免正文中的产品、地区、内存、价格或供应背景桥接无关事件。
- 保持仅依据历史日历推演的媒体发布日期预测处于 `deferred_events`，同时保留具有当前明确消息源的发布时间报道。
- 更新手动兜底规则，并将回归测试从 829 项增加到 841 项；并行联网验证保持候选、详情选择和文章覆盖完全一致，没有聚类警告，同窗口运行时间由基线 170.01 秒降至 152.45 秒。

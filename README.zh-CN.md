...
- 改进高新闻量场景下的实时抓取性能，使用详情页并发抓取，并缓存重复的词项、评分和主主题 facet 计算。
- 改进 `26.6` 和 `27` 测试版系列、macOS 壁纸、watchOS Siri AI、Siri 语音自定义以及平台特定测试版细节的系统版本边界。
- 改进 App Store / iCloud 支付方式恢复、账单可用性，以及 Apple TV app / iTunes Store 已购内容 4K 升级等 Apple 服务处理。
- 改进 Broadcom 供应协议、折叠 iPhone 量产、Apple Watch 端侧 AI 出货报告、iPhone 电池泄露、iPhone Air 2 传闻和 Mac AI 需求报告的硬件与市场聚类。
- 更新弱相关过滤、兜底策略说明和回归测试，同时保持发现范围宽泛，并继续将可复核的弱相关项目保留在 `deferred_events`。

...
- 新增更细的服务与内容主题边界，覆盖 Apple TV 体育赛程、动漫展阵容、预告片、Siri AI 和解、印度关税制造政策和端侧 AI 模型压缩，避免高新闻量时被合并成宽泛事件。
- 改进 iPhone 电池、重量、组件成本、折叠 iPhone、iPhone Ultra 定价、屏幕平整度、机模和量产传闻的硬件聚类，按具体 Apple 动作拆分同一产品线下的不同消息。
- 修复中文“前苹果”背景识别，避免 `目前苹果官网` 这类普通短语把直接 Apple 硬件定价报道误降级，同时保留真实前 Apple 员工评论的 `weak` 判定。
- 收紧未来 Apple 产品价格预测识别，要求明确价格或起售价锚点，避免渲染图、数据泄露、面板和供应链事件被价格主题污染。
- 更新兜底规则文档，并增加 deferred 拉回、第三方配件过滤、来源尾部清理、服务内容合并和硬件传闻边界的回归测试。
- 保持联网验证中的抓取覆盖稳定和近期性能目标，同时将 iPhone Ultra 定价传闻从 `deferred_events` 正确拉回 `events`。

...
- 新增更强的最终简报覆盖元数据，写入 `final_brief_queue`、`required_final_brief_titles`、`final_brief_coverage`、状态 JSON 和相邻 `*.brief.md` 脚手架，使每个合格 JSON 事件都保持硬性简报边界。
- 改进省略规则提示，避免自动化 agent 仅因条目是单来源、传闻表述、优先级较低、带有竞品背景，或不如当天大新闻显眼，就漏掉已进入 `events` 的合格项目。
- 更新 skill 与兜底规则文档，明确当抓取器已经将 Apple 硬件路线图或产品开发传闻纳入事件后，单来源报道也不应被静默省略。
- 增加单来源传闻型硬件事件的回归测试，例如 MacRumors 的 Apple iRing 报道，确保这类事件会出现在必收简报清单中。

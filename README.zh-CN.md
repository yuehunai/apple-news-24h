...
- 新增 IT之家列表页元数据抽取，让 Apple 频道和标签页能把文章摘要与 `data-ot` 发布时间带入候选过滤。
- 改进重复候选处理，同一 URL 会保留信息最完整的发现项，而不是优先保留首页里的稀疏链接。
- 改进 WWDC 相关性和详情页优先级，避免大会材料、参会礼品、徽章、贴纸、Developer app、吉祥物和官方活动报道被当作轻量内容漏掉。
- 改进 IT之家正文清理，过滤固定广告声明，避免其进入摘要和 `key_facts`。
- 更新 skill 和来源策略指引，要求等待较长的实时抓取完成，包括超过 5 分钟的运行，再判断结果或进入 diagnostics。
- 增加 IT之家列表摘要、信息更完整的重复候选、WWDC 优先级、广告声明过滤和相关文章噪音抑制的回归测试。

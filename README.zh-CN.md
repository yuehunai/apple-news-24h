...
- 新增多产品硬件路线稿的来源报告血缘，仅在不同语言的父报告独立覆盖多个相同子边界后才进行归并，并保留产品及代际级主张归属。
- 收紧聚合路线稿与聚焦发布、量产、价格、取消、保留和功能报道之间的结构化归并，避免单个共享产品或孤立事实桥接不同动作，同时保留真正同动作的完整跨来源覆盖。
- 改进标题与导语主张归属及同产品家族的分段解析，使后续事实持续归属于对应型号或代际；通过系统代码曝光的 Apple 第一方硬件保持强相关硬件新闻，第三方应用或配件继续留在延后队列。
- 修复 Apple Watch 硬件路线与 watchOS 功能移除被错误聚类的问题，并将 Apple 自研游戏手柄披露恢复为强相关硬件事件，保留完整跨来源归属。
- 更新投影血缘、第一方实体硬件、服务内容边界和第三方 Apple 平台上下文的兜底规则说明，保持 skill 工作流程、CLI、来源列表和详情页上限不变。
- 新增 34 项聚焦回归测试，将测试总数由 1,421 项增加到 1,455 项；并发联网验证耗时为 98.56/99.62 秒，主来源发现范围未回退；同输入重放保留全部 74 个来源 URL 且聚类警告为 0；纯净子代理验证得到 41 个主事件和 22 个延后事件，79 个保留来源链接均精确覆盖，无需人工重新聚类或提升事件。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260905_claim_ownership_boundaries.py` 的按日期命名测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、发布会活动、第一方设施、活动解读、内容形态边界、相关性分层、产品生命周期、实际部署、量化公司报告、详情页准入和官方叙述事实提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

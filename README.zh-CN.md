...
- 新增基于标题与导语的 Apple 产品量化应用研究、Apple 产品驱动型市场预测、具体发布会运营和第一方紧急组件采购事件身份，确保直接硬件与公司动作保持可收录。
- 改进版本化系统功能报道、iCloud Private Relay 泄露、九月发布会员工支持与演示形式、跨语言折叠屏市场预测的确定性重整，同时保留所有匹配来源。
- 改进供应链边界，使 Apple 与长鑫存储的直接谈判和紧急 DRAM 采购保持强相关，将宽泛内存市场背景拆开，并让由非 Apple 业务驱动的供应商财报在未来 iPhone 仅作为背景时保持弱相关。
- 改进来源主事实抽取和中文句子切分，避免相关文章文本提升或归并无关报道，同时让来源特有数字和直接动作继续进入最终摘要素材。
- 收紧运营商优惠、第三方 Mac 工具、配件清单、历史发布会日期推测、产品评论和宽泛行业背景的弱事件判断，同时不减少候选发现或详情页覆盖。
- 更新手动兜底与架构文档，并将回归测试从 891 项增加到 903 项；并发联网验证保持候选、详情选择、文章和 62 个 URL 覆盖完全一致，运行时间仍处于 120 秒目标内。

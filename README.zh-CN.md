...
- 新增 Apple 芯片关税豁免的跨角度归并，在监管与代工报道共享同一供应商、生产承诺和豁免行动时合并为同一事件。
- 改进同案法律报道归并，优先依据具体当事方和诉讼程序合并回应与背景报道，避免正文中的无关背景主题将其拆开。
- 收紧没有新增 Apple 行动的多产品前瞻、第一人称 Apple 设备或企业部署体验，以及病毒式 AI 生成 Apple 产品图片辨伪内容过滤。
- 保持 Apple Books 和 App Store 中 AI 仿冒、欺诈、冒充及版权问题作为平台信任新闻的纳入资格，同时将重复利用产品传闻的背景内容移出必收事件。
- 更新兜底规则文档并将回归测试扩展至 482 项；联网对比中各来源覆盖完全一致，抓取耗时进一步改善。

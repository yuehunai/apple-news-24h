...
- 改进 Apple 官方商店配件处理，使通过 Apple 渠道新增上架的第三方配件归为硬件/产品新闻，同时避免放大普通零售促销。
- 收紧 Apple 服务和内容事件聚类，使 Apple TV 续订、Apple Music 歌单、Apple One 优惠和信用卡权益保持独立，除非共享标题级实体。
- 改进 9to5Mac 和 IT之家来源清理，在生成摘要和 `key_facts` 前移除更多推广区块、通用服务订阅模板和无关页面/侧栏内容。
- 修复 IT之家 `data-ot` 等七位小数 ISO 时间字段的解析。
- 保留 Apple 服务优惠中的关键事实，包括订阅价格、折扣、资格条件和信用卡权益，同时过滤通用订阅模板。
- 增加回归测试，覆盖官方配件可用性、服务事件拆分、来源清理、服务优惠事实、IT之家正文范围和长 ISO 时间解析。

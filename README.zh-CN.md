...
- 新增更强的直接 iPhone 硬件传闻判断，使电池容量、SIM/eSIM 和供应商泄密细节留在 `events`，不再误入 `deferred_events`。
- 改进折叠 iPhone 面板市场聚类，使 Counterpoint 风格的面板份额和出货报告可跨源合并，同时与产量目标、价格和路线图报道保持分离。
- 修复 MacBook 涨价和出货市场报告被并入 M 系列 MacBook 路线图事件的问题。
- 更新兜底策略说明，补充 iPhone 硬件规格、折叠面板市场边界和 MacBook 价格/路线图拆分规则。
- 增加 iPhone 泄密细节、折叠面板报告、直接 iPhone 电池传闻和 MacBook 价格/路线图边界回归测试。

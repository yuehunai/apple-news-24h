...
- 恢复标题明确指向 Apple 且具有实质报道内容的详情页准入，不再依赖旧动作词或狭窄事件类型列表，同时将准入与最终相关性分开判断，继续拒绝仅含产品名的导航项。
- 改进文章形态与动作归属识别，保留聚焦的技术披露、大小写混合的功能名和当前有来源依据的报道，同时将旧传闻汇总、编辑前瞻、历史对比及零售店顾客轶事保留在延后队列。
- 改进命名内容作品、带版本号的跨设备能力及具体机型配色披露的跨语言归并，保持预告与影评、公司与高管账号变动、当前应用分发政策与历史支持时间线各自独立。
- 收紧产品路线图证据，将发布动作和年份绑定到同一主要陈述，避免借用另一厂商的采用时间或背景中的 Apple 发布信息错误提升竞品报道。
- 改进新发布的 Apple 官方沟通内容的事实保留，在数字与列表抽取未得到事实时补充实质性非数字叙述，并更新兜底策略说明，保持 skill 工作流程、CLI、来源列表和详情页上限不变。
- 新增 57 项聚焦回归测试，将测试总数由 1,330 项增加到 1,387 项；并发联网验证完整保留基线的 63 个来源 URL，额外恢复 6 个，耗时分别为 92.4/92.5 秒，纯净子代理验证无需人工重新聚类或从延后队列提升事件。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_reconciliation_20260903.py` 的按日期命名测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、发布会活动、第一方设施、活动解读、内容形态边界、相关性分层、产品生命周期、实际部署、量化公司报告、详情页准入和官方叙述事实提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

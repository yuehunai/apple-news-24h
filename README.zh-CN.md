...
- 新增由标题和导语驱动的类型化事件身份，覆盖主体、谓词、产品与平台、组件、证据、说法生命周期、法律阶段、系统发布波次及市场事实，在不依赖模糊桥接的情况下确定性拆分和归并。
- 改进供应链聚类，区分 Apple 供应商采购、测试与认证、出货计划调整及共享短缺背景；供应商主语的直接 Apple 硬件报道保持强相关并完整归并来源。
- 改进精确事件边界，避免正文后段、相关文章、共同原因及弱第三方、竞品或评论内容重定义或进入主事件，并新增 9to5Mac 认证翻新折扣尾注清理。
- 改进运行效率，以保持词边界等价的缓存匹配和预编译身份模式替代高频正则扫描；高新闻量联网处理由 160.79 秒降至约 90 秒，同时保持来源计数与 URL 覆盖不变。
- 更新手动兜底规则和架构文档，将回归测试由 936 项增加到 982 项；并发联网验证完整保留 116 个 URL 且无聚类警告或重复归属，纯净子代理验证得到 30 个一一对应的简报边界和完全一致的来源 URL。
...
- `tests/test_event_identity_architecture.py`、`tests/test_20260805_event_reconciliation.py`、`tests/test_20260806_relevance_boundaries.py` 和 `tests/test_20260811_structured_reconciliation.py` 为身份提取、匹配、结构化重整、供应商采购和相关性边界提供聚焦的回归测试；现有爬虫测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

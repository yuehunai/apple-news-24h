...
- 新增操作系统版本、发布通道、测试版编号、诉讼对手方、战略交易、股价变动、恶意软件家族和硬件动作等权威事件身份，避免无关报道因泛化共享词发生传递式误聚类。
- 改进高新闻量下 Apple 收购、公测版与开发者测试版、市场份额、Apple TV 体育内容、股价变动、芯片代工、安全报告和硬件诉讼的跨源覆盖，并在最终简报中保留每个经审阅的来源 URL。
- 改进系统发布边界，按具体 Apple 发布动作识别和归并首个公测版、替换后的开发者版本、带编号的测试版批次、内部测试、当前功能指南及建议或汇总文章。
- 收紧独立开发者项目、常规第三方 Apple 平台应用、竞品主导的排名与元件报道、产品名称碰撞，以及仅把 Apple 作为背景的非 Apple 诉讼等弱相关内容过滤。
- 更新兜底规则和面向 agent 的来源覆盖约束，将回归测试扩展至 528 项，并通过联网验证确认目标来源完整、事件边界清晰。

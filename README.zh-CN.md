...
- 改进非 Apple 主体和 Apple 相似词的弱相关过滤，例如 Safari 车型、非 Tim Cook 的地名、第三方芯片发布、Plex 调价和 Mac 上的第三方浏览器使用体验等不再误入最终事件。
- 改进高涨价密度下的硬件聚类，将受限内存供应商审批、外部回应、供应商成本转嫁争议、官翻上架、未来产品涨价预测和广义元器件短缺背景拆成独立事件族。
- 改进 `key_facts` 抽取，使短结构化列表项在事件级摘要中保留下来，例如 iPadOS 不再支持机型这类完整设备列表。
- 改进 MyDrivers/快科技 来源清理，避免尾部相关资讯块污染摘要、关键事实或事件归并锚点。
- 增加非 Apple 附带上下文、Apple 相似词、内存供应商审批、涨价后续拆分、短列表事实和 MyDrivers 相关资讯清理的回归测试。

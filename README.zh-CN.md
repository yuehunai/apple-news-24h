...
- 改进 9to5Mac 发现流程，保留 WordPress API 返回的分类和标签上下文，恢复 Apple TV+ 内容报道和系统更新报道的覆盖。
- 改进系统版本发布聚类，使当前测试版、旧版本 RC、公开安全更新和支持文档解释仅在描述同一轮发布时才合并。
- 改进硬件路线图和数据泄露聚类，使 A20 Pro 封装、iPhone 18 Pro 功能传闻、iPhone Ultra 或 iPhone Air 路线图、供应商泄露、涨价和内存供应故事在具体 Apple 动作不同时保持拆分。
- 新增爱范儿 `早报` 来源级过滤，并保持 IT之家 `IT早报` 过滤规则一致，降低每日早报汇总噪音，同时不缩窄独立 Apple 正文报道发现范围。
- 更新 skill 与兜底策略说明，并增加必收简报覆盖、Apple 服务/内容收录、系统发布边界、战略交易和高流量路线图聚类的回归测试。

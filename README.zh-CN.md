...
- 新增针对具体系统组件、Apple TV 内容标题与生命周期动作、Mac 路线图芯片代际、生产障碍、显示面板供应商议价、设施事故、监管反制和 Apple 平台安全报道的标题与导语语义边界。
- 改进假冒 App Store 钱包诉讼、防水 iPad mini 路线图、macOS Mail 变化、Siri 和解、显示面板议价、Apple Maps 集成及 Apple 存储供应商游说的跨来源、跨语言聚类，同时保留所有提供事实的来源链接。
- 新增复合服务内容文章拆分和不可回并的同源变体边界，并通过归一化发布日期与集数证据保持不同作品独立，同时合并同一作品的翻译报道。
- 收紧发现、正文提取和相关性判断：在详情抓取前过滤 MacRumors `Top Stories:` 汇总，在页面组件污染正文时优先使用权威结构化正文，并将没有新 Apple 动作的教程、传闻汇总、购买建议、第三方兼容性报道和评论分析降级。
- 更新手动兜底策略，并将回归测试从 706 项增加到 741 项；并行联网验证除有意排除的 `Top Stories` 汇总外保留了全部有效来源 URL，事件边界干净且运行时间没有实质回退。

...
- 改进种子组与结构化档案边界的对称性，避免产品路线图、高管接任、合作方设施和产品功能因文章顺序或共享 Apple 背景而产生不同聚类结果。
- 新增配件兼容性评估、组件采用、多产品路线图推演、CEO 交接、合作方创新设施及重启独立维修诉讼的结构化跨语言事件身份。
- 改进基于标题和导语的动作归属判断，使第三方 Apple 平台应用更新继续留在延后队列；只有 Apple 改变 API、政策、互操作行为、安全边界或第一方能力时才可提升。
- 更新设施、法律阶段、多产品及第三方平台事件边界的兜底规则。
- 新增 8 项聚焦回归测试，将测试总数由 1,271 项增加到 1,279 项；并发联网验证完整保留 42 个已审查 URL，运行时间由 71.96 秒降至 66.89 秒；纯净子代理验证生成 16 个主事件、9 个延后事件、零条合并警告，且无需人工修正事件边界。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260831_structured_event_repair.py` 的按日期命名协调测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、发布会活动、第一方设施、活动解读、内容形态边界和相关性分层提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

...
- 新增由标题主导的确定性事实签名，覆盖精确配色集合、表壳材质、形态重构、Apple Upgrade 具体操作、未来产品代际和第一方文档生命周期，在不重新启用模糊聚类的前提下改进多语言同事件归并。
- 改进第一方支持页面和使用手册的生命周期处理，将首次发布及其直接删除、更正、恢复或替换保留为一个按时间推进的事件，同时让后续发布、批准和政策动作保持独立。
- 改进折叠 iPhone 配色与路线图、Apple Watch 形态重构与陶瓷表壳报道，以及有明确来源的 Apple 硬件规划边界，避免背景中的备选方案重新定义主事件。
- 收紧犯罪汇总、基于旧传闻推演的评论文章和仅以 Apple 为参照的非 Apple 项目相关性判断，并避免将泛化的 Apple 删除措辞误判为 App Store 下架。
- 更新 skill 与手动兜底规则，并将回归测试从 928 项增加到 936 项；并发联网验证保持相同的 42 个来源 URL 并集、聚类警告为零，运行时间由 101.30 秒降至 97.88 秒。

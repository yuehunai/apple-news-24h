...
- 提升标题与导语结构化裁决对旧召回证据的抗干扰能力，在最终协调前拆开第一方发布、能力变化、价格、配置、市场报告，以及编辑性或第三方背景。
- 新增面向复合 M6 与 M5 Ultra 公告的 Apple 芯片主体级投影，分别保留各芯片自身事实和来源归属，避免共享发布表述桥接芯片、设备或价格事件。
- 改进当前 OS 功能矩阵、Home Hub 与辅助功能报道、外接显示能力、具名内容作品、产品发布及实测全球市场报告的精确跨语言协调，同时继续分离不相关动作。
- 收紧推断发布日期、基于历史节奏的预测、购物与对比文章、编辑汇总及弱背景提及的延后处理；最终简报不再从弱汇总正文中挖取独立事实。
- 更新架构说明并将回归测试由 1,190 项增加到 1,229 项；并发联网验证在约 79 秒内保持发现、详情选择、文章及 110 个唯一来源 URL 的覆盖完全一致，纯净子代理验证产生零条合并警告并覆盖全部 88 个主事件来源链接。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260826_structured_authority.py` 的按日期命名协调测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、种子证据、内容形态边界和相关性分层提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

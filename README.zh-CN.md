...
- 新增多产品 Apple 硬件报道的结构化投影与归并，将每个事件绑定到具体产品、动作、芯片代际或型号代码、发布/取消/保留状态和报道周期，使不同 Mac 路线图动作保持独立，同时归并精确对应的跨语言报道。
- 改进标题与导语的动作归属及内容形态判断，保留涉及 Apple 运营账户、服务、支付和储值系统的安全事件及当前第一方产品、供应链报道，同时将教程、倒计时、传闻目录、第三方 HomeKit/配件/平台发布和借 Apple 同款方案作比较的竞品报道保留在延后队列。
- 改进价格、产量、供应商和硬件配置边界，引入具名报告归属及证据范围内的事实归属，在保留信息较少的跨语言来源时，避免报告名、共享组件、背景产品或推测性从句桥接不同事件。
- 清理重复的旧 Mac 路线图及高管策略运行路径，让结构化归并对旧召回种子执行统一门禁，仅保留移除后会造成大范围跨语言召回回退的旧种子行为。
- 更新动作归属、第三方 Apple 平台上下文、Apple 运营系统安全事件、多产品投影和结构化冲突处理的兜底规则说明，保持 skill 工作流程、CLI、来源列表和详情页上限不变。
- 新增 34 项聚焦回归测试，将测试总数由 1,387 项增加到 1,421 项；并发联网验证保持发现、详情选择、文章数量及全部 75 个来源 URL 完全一致，耗时由 94.11 秒降至 90.31 秒；纯净子代理验证得到 21 个主事件和 29 个延后事件，无聚类警告，也无需人工重新聚类或提升事件。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260904_projection_and_service_security.py` 的按日期命名测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、发布会活动、第一方设施、活动解读、内容形态边界、相关性分层、产品生命周期、实际部署、量化公司报告、详情页准入和官方叙述事实提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

...
- 新增 9to5Mac 及类似页面的优先正文区域抽取，避免相关文章卡片或侧栏内容替代真实正文。
- 新增已入选候选在详情页抓取失败时的 diagnostics 和低置信发现兜底，保留带可解析时间的 feed / 列表页有效信息。
- 改进高新闻量下的 OS 和 WWDC 覆盖，将详情页抓取上限提高，并补强新增功能、兼容性变化、自定义选项和 developer beta 等动作匹配。
- 改进短列表和表格行的 `key_facts` 抽取，让功能列表、实现选项和本地化产品细节能进入摘要。
- 更新 skill 和来源策略指引，将 JSON `events` 视为合格事件队列，保留细粒度产品 / 平台变化，并保留贡献独立细节的来源链接。
- 增加优先正文区域选择和候选入选后详情页失败兜底的回归测试。

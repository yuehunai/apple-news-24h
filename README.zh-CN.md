...
- 新增第一方发布会活动身份，将官方活动公告、本地化口号与日期及社交平台 hashmoji 激活精确归并，同时继续把生产报道、时间预测和购买意向调查保留为独立动作。
- 改进以标题主体和动作作为裁决依据的精确跨来源协调，覆盖聚焦搜索变化、Wallet 文档支持、Mac mini 待交付订单免费升级、折叠设备组件泄露、Apple TV 演员公告、Apple 支持 AI、Genlock、实测市场结果及法律案件排期。
- 新增对暂时失败的已选详情页执行一次有界二次抓取，在不重复抓取成功页面的情况下保持发现覆盖和诊断信息完整。
- 收紧编辑性对比、概念文章、推测日期、多厂商汇总和购买意向调查的相关性处理，使弱背景报道继续可供复核但不进入必收简报。
- 更新兜底规则并将回归测试由 1,229 项增加到 1,247 项；并发联网验证在约 96 秒内保持发现、详情选择、文章及全部 115 个来源 URL 的覆盖一致，纯净子代理验证产生零条合并警告，并将购买意向调查保留在 41 个事件的延后审查账本中。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260827_event_campaign_boundaries.py` 的按日期命名协调测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、发布会活动、种子证据、内容形态边界和相关性分层提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

...
- 新增命名 Apple Silicon 兼容项目、Apple 硬件组件维修费用估算，以及未发布第一方硬件发布路线图的结构化跨语言事件身份。
- 改进动作归属判断，使带版本号的 Apple 系统或 CarPlay 名称不能单独提升第三方应用可用性报道；只有 Apple 或平台引入明确 API、政策、互操作规则或第一方能力变化时才可提升。
- 改进内容形态边界，使发布会看点汇总、第一人称产品使用文章及单用户电池或售后资格实验继续留在延后队列，同时不压制直接 Apple 行动。
- 收紧维修金额与未来路线图的结构化证据，避免整机价格、竞品对比、历史日期及多产品背景参与事件身份判断。
- 更新兜底规则并将回归测试由 1,264 项增加到 1,271 项；并发联网验证未丢失任何基线 URL，新增发现 1 个来源 URL，运行时间为 65.57/65.95 秒；纯净子代理验证生成 8 个主事件、13 个正确延后事件和零条合并警告。
...
- `tests/test_event_identity_architecture.py` 及截至 `tests/test_20260830_structured_identity_and_relevance.py` 的按日期命名协调测试，为身份提取、匹配、结构化断言归并、动作归属、主张投影、变更对象边界、发布会活动、第一方设施、活动解读、内容形态边界和相关性分层提供聚焦回归覆盖；现有抓取器测试则继续端到端验证发现、解析、聚类、渲染和来源清理。

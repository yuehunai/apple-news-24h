...
- 新增 `scripts/apple_news_core` 包：`event_identity.py` 根据标题和导语为产品、组件、参与方、动作、地区、法律案件、内容形态和命名主体构建语义事件身份；`event_matcher.py` 则在爬虫编排层之外，以纯函数执行保守的同事件兼容性判断。
- 改进操作系统发布批次、第一方应用与服务、法律和公司动作、区域可用性、硬件路线图、供应商、价格、市场报告及内容标题的事件边界，使同一动作的报道保留全部来源链接，同时避免无关报道因宽泛 Apple 上下文而误聚类。
- 改进公司设施、平台信任、生态互操作、研究和市场报告等直接 Apple 事项的相关性及延后事件处理，同时继续阻止教程、投票、播客、例行优惠、竞品比较、联盟营销内容和偶然提及 Apple 的报道进入必收事件。
- 改进来源正文清理和事实提取，移除相关文章、联盟营销、反馈提示及纯界面导航噪声，同时保留各来源提供的具体数字、完整型号或地区表格、功能列表、官方解释及其他重要 `key_facts`。
- 新增独立的事件身份架构测试，将回归测试从 581 项扩展至 665 项；缓存与联网验证保持候选、文章及 URL 覆盖不缩窄，并且没有产生聚类警告。

...
- 改进 Apple 主导的开发者提案征集准入与相关性判断，共用标题和导语规则，恢复浏览器互操作性征案的来源覆盖，避免合作方或竞品背景覆盖主动作。
- 改进带版本的系统代码披露、平台获奖总量、供应商合作回应、长期组件合约及具名机构产量预测的跨语言归并，将事件身份绑定实际主体、动作、报道周期和支持证据。
- 保持明确构建版本冲突对归并的约束力，区分货币金额与日历年份，并完整解析含千分位的价格，避免无关系统版本和价格报道产生错误事件边界。
- 改进第一方应用公告和具体跨平台互通的动作归属，保持第三方应用上架、购买推荐、竞品营销及无新证据评论处于延后队列。
- 更新兜底规则及共享判断辅助函数，保持 skill 工作流程、CLI、来源列表、详情页覆盖和输出结构不变。
- 新增 58 项聚焦回归测试，将测试总数由 1,471 项增加到 1,529 项；并发联网验证保留全部 57 个基线来源 URL，并恢复一个来源，耗时为 80.21/79.33 秒；纯净子代理输出 17 条简报，完整保留 36 个有效来源链接，无需人工重新聚类或提升延后事件。
